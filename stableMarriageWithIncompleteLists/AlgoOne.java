...
public class AlgoOne {
	...
	/*
	 * we say that personpisacceptableto personqifpappearson the preference list
	 * ofq, andunacceptableotherwise
	 */

	/*
	 * Mis stable if there is no(man,woman) pair (m;w), each of whom is either
	 * unmatche dinMan d6n ds the otheracceptable, or prefers the other to his=her
	 * partner inM
	 *
	 *
	 *
	...
	public void algorithm() {
		for (int i = 0; i < totalMen; i++) {
			for (int j = 0; j < totalMen / 2; j++) {
				...
				if (choice != 0) {
					if (womenSingles[choice - 1] == false) { // if woman is
// single
						if (accepts(i + 1, choice)) {
// if man engaged, check if his current partner
// prefers his ass more than the next choice
							if (manNotAvailable[i] == true) {// if man married,
// then check
// his ranks
...
// if yes then, engage them and record her rank of him and
// add it to married man
...
// put that man with no preference list in singles
			...
			if (manNotAvailable[i] == false) { // if man never got married even
// in his first half list, put
// him in singles list
				menSingles.add(i + 1);
				manNotAvailable[i] = true;
			}
